...
EXEC_TARGET fptype device_Gaussian_Point (fptype* point, fptype* p, unsigned int* indices) {
  fptype x = point[0];
...
MEM_DEVICE device_function_ptr ptr_to_Gaussian_Point = device_Gaussian_Point;
  ...
  : GooPdf(_x, n)
{
  ...
  GET_ATPOINTS_ADDR(ptr_to_Gaussian_Point);
  initialise(pindices);
}
