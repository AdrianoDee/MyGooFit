#include "MultiDimHistoPdf.hh"
#define JUMP 22222222
MEM_CONSTANT fptype* dev_base_multidimhisto[100]; // Multiple histograms for the case of multiple PDFs

     EXEC_TARGET fptype device_MultiDimHistoPdf(fptype* evt, fptype* p, unsigned int* indices) {
       // Structure is
       ...
       fptype* myHistogram = dev_base_multidimhisto[myHistogramIndex];
       fptype ret = 0;
...
MEM_DEVICE device_function_ptr ptr_to_MultiDimHisto = device_MultiDimHistoPdf;

__host__ MultiDimHistoPdf::MultiDimHistoPdf (std::string n,
							 BinnedDataSet* x,
  ...
  , numVars(x->numVariables())
{
  ...
  MEMCPY_TO_SYMBOL(dev_base_multidimhisto, dev_address, sizeof(fptype*), totalHistograms*sizeof(fptype*), cudaMemcpyHostToDevice);
  // printf("NumVars Declaration = %d \n",numVars);
  GET_FUNCTION_ADDR(ptr_to_MultiDimHisto);
  initialise(pindices);
