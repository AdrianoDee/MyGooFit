...
MEM_CONSTANT fptype wLeg[8] = {0.362683783, 0.3137066458,0.222381034,0.101228536,0.3626837833, 0.3137066458,0.222381034,0.10122853};
MEM_CONSTANT fptype xLeg[8] = {0.183434642,0.5255324099,0.7966664774,0.9602898564,-0.183434642,-0.525532409,-0.796666477,-0.9602836};
...
EXEC_TARGET fptype calculateBinInt (fptype rawPdf, fptype* evtVal, unsigned int par) {
  rawPdf *= normalisationFactors[par];
  //rawPdf *= evtVal[1]; // Bin volume
  //rawPdf *= evtVal[1];
  // Log-likelihood of numEvents with expectation of exp is (-exp + numEvents*ln(exp) - ln(numEvents!)).
...
MEM_DEVICE device_metric_ptr ptr_to_BinInt       = calculateBinInt;
...
MEM_DEVICE device_metric_ptr ptr_to_BinVol       = calculateEval;
...
__host__ void GooPdf::initialiseInt (std::vector<unsigned int> pindices, void* dev_functionPtr, void* dev_integralPtr,void* dev_pointPtr) {
  if (!fitControl) setFitControl(new BinnedNllFitInt());

  // MetricTaker must be created after PdfBase initialisation is done.
  PdfBase::initialiseIndices(pindices);

  functionIdx = findFunctionIdx(dev_functionPtr);
  functionIntegralIdx = findFunctionIdx(dev_integralPtr);
  functionPointIdx = findFunctionIdx(dev_pointPtr);
...
__host__ void GooPdf::setMetricsInt () {
  if (logger) delete logger;
  logger = new MetricTaker(this, getMetricPointer(fitControl->getMetric()),1);
}
...
__host__ void GooPdf::evaluateAtPointsInt (Variable* var, std::vector<fptype>& res) {
  // NB: This does not project correctly in multidimensional datasets, because all observables
  // other than 'var' will have, for every event, whatever value they happened to get set to last
  // time they were set. This is likely to be the value from the last event in whatever dataset
  // you were fitting to, but at any rate you don't get the probability-weighted integral over
  ...
  MetricTaker evalor(this, getMetricPointer("ptr_to_Eval"),1);
  thrust::transform(thrust::make_zip_iterator(thrust::make_tuple(eventIndex,arrayAddress,eventSize)),
		    thrust::make_zip_iterator(thrust::make_tuple(eventIndex + numEntries,arrayAddress,eventSize)),
		    results.begin(),
		    evalor);
  ...
  for (int i = 0; i < var->numbins; ++i) {
    res[i] = h_results[i] * host_normalisation[parameters];
  //  printf("%d --- resi = %f hresi = %f norm = %f\n",i,res[i],h_results[i],host_normalisation[parameters]);
  }
...
__host__ fptype GooPdf::normaliseInt () const {
  //if (cpuDebug & 1) std::cout << "Normalising " << getName() << " " << hasAnalyticIntegral() << " " << normRanges << std::endl;
...
EXEC_TARGET fptype MetricTaker::operator () (thrust::tuple<int, fptype*, int> t) const {
  ...
  // Causes stack size to be statically undeterminable.
   fptype ret;
   if (intFlag == 1) ret = callFunction(eventAddress, functionIntegralIdx, parameters);
   else ret = callFunction(eventAddress, functionIdx, parameters);
...
//ONLY FOR BINNED & BIN INTEGRAL!!!
EXEC_TARGET fptype MetricTaker::operator () (thrust::tuple<fptype*,int, int> t) const {
    // Calculate event offset for this thread.
  int eventIndex = thrust::get<1>(t);
  int eventSize  = thrust::get<2>(t);
  fptype* eventAddress = thrust::get<0>(t) + (eventIndex * abs(eventSize));

  // Causes stack size to be statically undeterminable.
   fptype ret;
   ret = callFunction(eventAddress, functionIntegralIdx, parameters);
...
__host__ void GooPdf::getCompProbsAtDataPointsUn (std::vector<std::vector<fptype> >& values) {
  ...
  for (unsigned int i = 0; i < components.size(); ++i) {
    MetricTaker compevalor(components[i], getMetricPointer("ptr_to_Eval"));
    thrust::counting_iterator<int> ceventIndex(0);
    thrust::transform(thrust::make_zip_iterator(thrust::make_tuple(ceventIndex, arrayAddress, eventSize)),
		      thrust::make_zip_iterator(thrust::make_tuple(ceventIndex + numEntries, arrayAddress, eventSize)),
		      results.begin(),
...
MetricTaker::MetricTaker (PdfBase* dat, void* dev_functionPtr)
  ...
  , functionIntegralIdx(dat->getFunctionIntIndex())
  , functionPointIdx(dat->getFunctionPntIndex())
{
...
MetricTaker::MetricTaker (PdfBase* dat, void* dev_functionPtr,unsigned int flagInt)
  : metricIndex(0)
  , intFlag(flagInt)
  , functionIdx(dat->getFunctionIndex())
  , parameters(dat->getParameterIndex())
...
__host__ void GooPdf::setFitControl (FitControl* const fc, bool takeOwnerShip) {
  ...
  if (fitControl->getMetric() == "ptr_to_BinInt" || fitControl->getMetric() == "ptr_to_BinVol") setMetricsInt();
  else setMetrics();
}
