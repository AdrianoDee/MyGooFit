...
EXEC_TARGET devcomplex<fptype> device_Faddeeva_4 (const devcomplex<fptype>& z) {
...
EXEC_TARGET fptype device_VoigtianRMS (fptype* evt, fptype* p, unsigned int* indices) {
  ...
  // actual VoigtianRMS for non-trivial width and sigma
  //fptype c = 1./(ROOT2*s);
  ...
  devcomplex<fptype> v = device_Faddeeva_4(z);
  
#define rsqrtPi 0.5641895835477563
  return c*rsqrtPi*v.real;
}

MEM_DEVICE device_function_ptr ptr_to_VoigtianRMS = device_VoigtianRMS; 

__host__ VoigtianRMSPdf::VoigtianRMSPdf (std::string n, Variable* _x, Variable* m, Variable* w)
: GooPdf(_x, n)
{
  std::vector<unsigned int> pindices;
  pindices.push_back(registerParameter(m));
  pindices.push_back(registerParameter(w));
  GET_FUNCTION_ADDR(ptr_to_VoigtianRMS);
  initialise(pindices); 
}
