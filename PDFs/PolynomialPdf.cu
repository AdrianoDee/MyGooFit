...
EXEC_TARGET fptype device_OffsetPolynomialCuts (fptype* evt, fptype* p, unsigned int* indices) {
    int numParams = indices[0];
    int lowestDegree = indices[1];

    fptype x = evt[indices[2 + numParams]];
    fptype lowerCut = p[indices[numParams-2]];
    fptype upperCut = p[indices[numParams-1]];

    if(x<lowerCut || x>upperCut) return 0.0;
...
// Constructor for single-variate polynomial, with optional zero point and cuts
__host__ PolynomialPdf::PolynomialPdf (string n, Variable* _x, vector<Variable*> weights,unsigned int flagCut,Variable* x0,Variable* lowerCut,Variable* upperCut, unsigned int lowestDegree)
: GooPdf(_x, n)
,center(x0)
...
,upperThreshold(upperCut)
{
    ...
    if (x0) {
        if (lowerCut && upperCut) {
            ...
            pindices.push_back(registerParameter(x0));
            GET_FUNCTION_ADDR(ptr_to_OffsetPolynomialCuts);
        }
        else{
            pindices.push_back(registerParameter(x0));
            GET_FUNCTION_ADDR(ptr_to_OffsetPolynomial);
