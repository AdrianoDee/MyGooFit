...
MEM_CONSTANT fptype Cc[7] = { 65536.0, -2885792.0, 69973904.0, -791494704.0,
			     8962513560.0, -32794651890.0, 175685635125.0 };
MEM_CONSTANT fptype Dd[7] = { 192192.0, 8648640.0, 183783600.0, 2329725600.0,
...
#ifndef UNROLL_LOOP
MEM_CONSTANT fptype Nn1[12] = { 0.25, 1.0, 2.25, 4.0, 6.25, 9.0, 12.25, 16.0, 20.25, 25.0, 30.25, 36.0 };
MEM_CONSTANT fptype Ee1[12] = { 0.7788007830714049,    0.3678794411714423,
...
MEM_CONSTANT fptype Nn2[12] = { 0.2809, 1.1236, 2.5281, 4.4944, 7.0225, 10.1124,
			 13.7641, 17.9776, 22.7529, 28.09, 33.9889, 40.4496 };
MEM_CONSTANT fptype Ee2[12] = { 0.7551038420890235,    0.3251072991205958,
...
EXEC_TARGET devcomplex<fptype> device_Faddeeva_3 (const devcomplex<fptype>& z) {
...
#endif
EXEC_TARGET fptype device_Voigtian_Thresh_Point (fptype* point, fptype* p, unsigned int* indices) {
 
  fptype x = *point;
  fptype m = p[indices[1]];
  fptype s = p[indices[2]];
  fptype w = p[indices[3]];

  //printf("Voigtian Thresh Point x = %f m = %f w = %f s=%f \n",x,m,w,s);
#ifdef CUDAPRINT
  ...
  //assert(w > 0);
  if (x<=1.01946 || x>= 1.688623) return 0;
  fptype arg = x - m;
  ...
  devcomplex<fptype> v = device_Faddeeva_3(z);
...
EXEC_TARGET fptype device_Voigtian_Thresh (fptype* evt, fptype* p, unsigned int* indices) {
  fptype x = evt[0];
...
EXEC_TARGET fptype device_Voigtian_Thresh_Bin (fptype* evt, fptype* p, unsigned int* indices) {
  ...
  if (Xs[k]<=1.01946 || Xs[k]>= 1.688623) fx[k]=0;
  // return constant for zero width and sigma
  if ((0==s) && (0==w)) fx[k]=1;
  //assert(s > 0); // No device-side assert?!
  //assert(w > 0);
...
MEM_DEVICE device_function_ptr ptr_to_Voigtian_Thresh = device_Voigtian_Thresh;
MEM_DEVICE device_function_ptr ptr_to_Voigtian_Thresh_Bin = device_Voigtian_Thresh_Bin;
MEM_DEVICE device_function_ptr ptr_to_Voigtian_Thresh_Point = device_Voigtian_Thresh_Point;


__host__ VoigtianThreshPdf::VoigtianThreshPdf (std::string n, Variable* _x, Variable* m, Variable* s, Variable* w)
: GooPdf(_x, n)
{
  ...
  GET_FUNCTION_ADDR(ptr_to_Voigtian_Thresh);
  GET_INTEGRAL_ADDR(ptr_to_Voigtian_Thresh_Bin);
  GET_ATPOINTS_ADDR(ptr_to_Voigtian_Thresh_Point);
