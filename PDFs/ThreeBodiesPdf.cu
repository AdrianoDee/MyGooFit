...
MEM_DEVICE device_function_ptr ptr_to_Three = device_Three;
MEM_DEVICE device_function_ptr ptr_to_Three_Bin = device_Three_Bin;
MEM_DEVICE device_function_ptr ptr_to_Three_Point = device_Three_Point;


__host__ ThreePdf::ThreePdf (std::string n, Variable* _x)
  : GooPdf(_x, n)
{
  std::vector<unsigned int> pindices;

  GET_FUNCTION_ADDR(ptr_to_Three);
  GET_INTEGRAL_ADDR(ptr_to_Three_Bin);
  GET_ATPOINTS_ADDR(ptr_to_Three_Point);
  initialiseInt(pindices);
}

__host__ fptype ThreePdf::integrate (fptype lo, fptype hi) const {
